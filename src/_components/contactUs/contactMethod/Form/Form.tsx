...
const ContactForm = () => {
  ...
  const [state, action, pending] = useActionState(manageForm, {
  ...
  return (
    <>
      <div
        className={`${snackBar === "success" || snackBar === "error" ? "flex" : "hidden"} bg-background/90 fixed top-25 z-50 items-center gap-3 rounded-xl px-4 py-3 shadow-lg backdrop-blur-md lg:top-20 ${
          snackBar === "error"
            ? "from-error-container/20 via-error-container/50 to-error-container/20 border border-red-500/20 bg-gradient-to-t"
            : "from-primary-container/15 via-primary-container/40 to-primary-container/15 border border-green-500/20 bg-gradient-to-t"
        }`}
      >
        <div
        ...
        <div>
          <p className="text-on-surface text-sm font-medium">
            {snackBar === "error" ? form.errorTitle : form.successTitle}
          </p>
        ...
        <div className="flex h-full w-full max-w-[1200px] flex-col items-center justify-center gap-4 md:flex-row md:items-start">
          <div className="border-outline/50 to-primary/5 w-[90%] items-center rounded-xl border-[1px] bg-gradient-to-tr px-6 py-8 sm:p-8 md:w-[70%] lg:my-0 xl:w-1/2">
            <div className="flex flex-col pb-4">
              ...
              <h2 className="text-on-surface-variant text-xl">
                {form.subtitle1}
              </h2>
              <h2 className="text-on-surface-variant text-xl">
                {form.subtitle2}
            ...
            >
                      ...
                      {isCountryOpen && filteredCountries.length > 0 && (
                        <div className="bg-surface-container-high border-primary/50 absolute top-full z-20 mt-1 max-h-60 w-64 overflow-y-auto rounded-lg border shadow-lg">
                          {filteredCountries.map((country) => (
                            ...
                            >
                              <div className="relative h-3 w-4 self-center">
                                <Image
              ...
              <label className="text-on-surface flex w-full flex-col gap-1">
                ...
                <p className="text-on-surface-variant text-right text-sm">
                  {brief.length ?? 0}/1750
                </p>
              </label>
